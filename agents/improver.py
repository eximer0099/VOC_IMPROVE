...
class PolicyImproverAgent:
    ...
    def __init__(self, model: str | None = None):
        ...
        # Anthropic이 빈 텍스트를 반환하거나 일시적으로 실패하면 정책 생성이
        # 안내 문구로 끝나지 않도록 별도 공급자 폴백을 사용합니다.
        self.fallback_llm = OpenAIChat(model=MODEL_SUMMARY)
...
"""
        ...
        try:
            ...
            log_authentication_error("Improver", exc)
            primary_error = exc
            text = ""
        ...
        if not policy_text or len(policy_text) < 20:
            policy_text = (await self.fallback_llm(prompt.strip()) or "").strip()

        if not policy_text or len(policy_text) < 20:
            detail = f" Primary provider error: {primary_error}" if primary_error else ""
            raise RuntimeError(f"Both policy generation providers returned no usable text.{detail}")

        # ============ 결과 반환 ============
        # 생성된 정책 개선안을 PolicyResult 객체로 감싸서 반환합니다
        agent_event("Improver", "completed", policy_length=len(policy_text))
        agent_file_event("Improver", "output", operation="improve", policy=policy_text)
...
"""
        ...
        agent_file_event("Improver", "output", operation="refine", policy=policy_text)
        return PolicyResult(policy=policy_text)
...
class ImproverServicer(voc_pb2_grpc.ImproverServicer):
    ...
    # ============ Improve RPC 구현 ============
    @log_response_time("Improver")
    async def Improve(self, request, context):
        """
        Improve RPC를 구현합니다.
        
        클라이언트로부터 최종 요약 텍스트를 받아 정책 개선안만 생성합니다.
        ...
        try:
            ...
            final_policy = final_policy.strip()
            agent_file_event("Improver", "output", rpc="Improve", policy=final_policy)
            return voc_pb2.PolicyRes(policy=final_policy)
        except Exception as e:
            log_authentication_error("Improver", e)
            # ============ 에러 처리 ============
    ...
    async def Refine(self, request, context):
        ...
        try:
            ...
            agent_file_event(
                "Improver", "output", rpc="Refine", policy=result.policy
            )
...
async def serve() -> None:
    ...
    bind_agent_port(server, bind, "Improver")

    # ============ 서버 시작 로그 ============
    # 서버가 시작되었음을 콘솔에 출력합니다
    agent_event("Improver", "server_started", bind=bind)
    # ============ 서버 시작 및 대기 ============
