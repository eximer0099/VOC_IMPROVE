...
class CriticAgent:
    ...
    async def review(self, doc: str, role: str) -> CriticResult:
        ...
        agent_event("Critic", "review", role=role, document_length=len(doc))
...
"""
        ...
        agent_event("Critic", "completed", role=role,
                    need_refine=need_refine, edit_count=len(edits),
        ...
        agent_file_event(
            ...
            operation="review",
            role=role,
            need_refine=result.need_refine,
            edits=result.edits,
            ask_more_samples=result.ask_more_samples,
        )
        return result
    ...
    async def Review(self, request, context):
        ...
        try:
            ...
            return voc_pb2.ReviewRes(
                ...
                policy=request.doc if role == "policy" else "",
                summary=request.doc if role == "summary" else "",  # 요약 (role="summary"일 때)
            )
        except Exception as e:
            log_authentication_error("Critic", e)
            # ============ 에러 처리 ============
...
async def serve() -> None:
    ...
    bind_agent_port(server, bind, "Critic")

    # ============ 서버 시작 로그 ============
    # 서버가 시작되었음을 콘솔에 출력합니다
    agent_event("Critic", "server_started", bind=bind)
    # ============ 서버 시작 및 대기 ============
