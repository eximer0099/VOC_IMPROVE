...
class EvaluatorAgent:
    ...
    @staticmethod
    def _normalize_candidate_key(raw_key, candidates: dict[str, str]) -> str:
        """Map common LLM winner formats (for example ``3``) to ``S2``."""
        keys = list(candidates.keys())
        if not keys:
            return ""

        value = str(raw_key or "").strip().strip('"\'')
        ...
        # Older prompts asked for 1/2/3 even though candidates use S0/S1/S2.
        # Preserve compatibility with those model responses.
        if value.isdigit():
...
winner와 scores의 키는 후보에 표시된 키(S0, S1, S2)를 그대로 사용해라.
출력 JSON 형식 예시:
{{
  "winner": "S2",
  "scores": {{
...
"""
        ...
        if not isinstance(data, dict):
            ...
            agent_file_event(
                "Evaluator", "output", operation="evaluate", **output
            )
            return output
        ...
        # 값이 없으면 기본값을 사용합니다
        winner = self._normalize_candidate_key(data.get("winner"), candidates)
        raw_scores = data.get("scores", {})
        scores = {}
        if isinstance(raw_scores, dict):
            for key, score in raw_scores.items():
                normalized = self._normalize_candidate_key(key, candidates)
                if normalized:
                    scores[normalized] = score

        output = {"winner": winner, "scores": scores}
        agent_file_event(
            "Evaluator", "output", operation="evaluate", **output
        )
        return output
...
class EvaluatorServicer(voc_pb2_grpc.EvaluatorServicer):
    ...
    async def Evaluate(self, request, context):
        ...
        """
        try:
            agent_event("Evaluator", "evaluate", task=request.task,
                        candidate_keys=list(request.candidates.keys()))
            # ============ 후보 딕셔너리 변환 ============
            ...
            # ============ 승자 키 검증 ============
            winner_key = self.agent._normalize_candidate_key(out["winner"], candidates)
            summary = candidates.get(winner_key) or next(
                (text for text in candidates.values() if text.strip()), ""
            )
            if not summary:
                raise ValueError("Evaluator received no non-empty summary candidates")
            agent_event("Evaluator", "completed", winner=winner_key,
                        scores=out["scores"])
            
            # ============ 응답 메시지 생성 및 반환 ============
            # 평가 결과를 gRPC 응답 메시지로 감싸서 반환합니다
            return voc_pb2.EvaluateRes(
                winner=winner_key,         # 정규화된 승자 키
                scores_json=json.dumps(out["scores"], ensure_ascii=False)  # 점수 정보를 JSON 문자열로 변환
            )

        except Exception as e:
            log_authentication_error("Evaluator", e)
            agent_event("Evaluator", "error", error=str(e))
            # ============ 에러 처리 ============
...
async def serve():
    ...
    bind_agent_port(server, endpoint, "Evaluator")

    # ============ 서버 시작 로그 ============
    # 서버가 시작되었음을 콘솔에 출력합니다
    agent_event("Evaluator", "server_started", bind=endpoint)
    # ============ 서버 시작 및 대기 ============
